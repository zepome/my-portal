...
function updateTodo() {
    ...
    if (todo.dueDate && isGoogleAPIConfigured()) {
        addToGoogleCalendar(todo).then(calendarSuccess => {
            if (calendarSuccess) {
        ...
        // Google Tasksにも更新（簡易的に新規追加として処理）
        addToGoogleTasks(todo).then(tasksSuccess => {
            if (tasksSuccess) {
                console.log('✅ Google Tasksに更新されました');
            } else {
                console.log('⚠️ Google Tasksへの更新に失敗しました。');
            }
        });
    }
}
...
async function initializeGapiClient() {
    ...
    try {
        ...
        gapiInited = true;
        console.log('GAPI client initialized (Gmail, Calendar, Tasks)');
        maybeEnableButtons();
    } catch (error) {
...
async function addToGoogleTasks(todo) {
    ...
    try {
        ...
        // 期限がある場合は設定
        if (todo.dueDate) {
            // Google Tasks APIはRFC 3339形式の日付を要求（YYYY-MM-DDT00:00:00.000Z）
